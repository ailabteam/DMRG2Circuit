...
import time
import seaborn as sns # Import seaborn để vẽ heatmap
...
def run_variational_experiment(L, J, g, num_layers, num_steps, step_size, alpha, num_runs=1):
    ...
    for run in range(num_runs):
        ...
        if final_fidelity > best_run_fidelity:
            ...
            # Lưu trữ các kết quả của lần chạy tốt nhất
            best_run_results = {
                "params": params,
    ...
    # Phân tích và lưu kết quả của lần chạy tốt nhất
    ...
    summary_metrics = {
        ...
        "fidelities_all_runs": all_fidelities,
        "fidelity_mean": np.mean(all_fidelities),
        "fidelity_std": np.std(all_fidelities)
    }
    ...
    np.save(f"{results_dir}/best_run_params.npy", best_run_results["params"])

    # Vẽ và lưu figures cho lần chạy tốt nhất
    plot_convergence(best_run_results["cost_history"], L, g, num_layers, f"{results_dir}/best_run_convergence.png")
...
def main():
    ...
    g_fixed = 0.8
    L_points = [6, 8] # Bắt đầu với các hệ nhỏ hơn trước
    for L in L_points:
        num_layers = 2 * L - 8  # Heuristic: 4, 8 lớp
        num_steps = 200 * (L - 4) # Heuristic: 400, 800 bước
